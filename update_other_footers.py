...
footer_content = """<!-- DETAILED CORPORATE FOOTER -->
...
html_files.remove('index.html') # We just updated this

for f in html_files:
    with open(f, 'r', encoding='utf-8') as file:
        content = file.read()
    
    # Use regex to find <!-- DETAILED CORPORATE FOOTER --> to </footer>
    pattern = re.compile(r'<!-- DETAILED CORPORATE FOOTER -->.*?</footer>', re.DOTALL | re.IGNORECASE)
    ...
    if pattern.search(content):
        ...
        print(f"Could not find footer in {f}")

print("Done replacing footers.")
